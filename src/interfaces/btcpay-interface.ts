...
export const createUserBtcpayApi = async (req: Request, res: Response): Promise<Response> => {
    const userEmail: string = await getUserEmailFromAccessTokenInAuthorizationHeader(req);
    try {
        console.log(`Starting creating BTCPAY user services for email ${userEmail}`);
        const createUserBtcpayDto: CreateUserBtcpayDto = req.body;
        await createUserBtcpayServices(userEmail, createUserBtcpayDto);
        console.log(`Successfully created user BTCPAY services for email ${userEmail}`);
        return res.status(200).send();
    } catch (err) {
        if (err instanceof UserBtcpayException) {
            return res.status(400).send(new ErrorDto(err.message, err.clientErrorCode));
        }
        console.log('Failed to add user BTCPAY services services', err);

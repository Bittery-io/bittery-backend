...
import { findAllLnds } from '../../../repository/lnd/lnds-repository';
...
export const startStaticChannelBackupScheduler = () => {
    ...
    schedule.scheduleJob('* * * * *', async () => {
        ...
        logInfo(`2/3 Obtained static channel backups for ${lndStaticChannelBackups.length}/${lnds.length} LNDs`);
        if (lndStaticChannelBackups.length > 0) {
            await insertLndStaticChannelBackups(lndStaticChannelBackups);
        }
        logInfo(`3/3 Successfully saved ${lndStaticChannelBackups.length} LNDs static channel backups!`);
    });
};

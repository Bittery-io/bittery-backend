...
//             exec(`${getProperty('BITTERY_INFRASTRUCTURE_PATH')}/sto
//             p-lnds.sh ${lndUserDomainNamesString}`, (error: any, stdout: any, stderr: any) => {
//                 if (error) {
...
//         const lndUserDomainNamesString: string = getLndUserDomainsString(lndRuns);
//         setTimeout(() => {
//             console.log(`Starting ${lndRuns.length} LNDs [${lndUserDoma
//             inNamesString}]. Starting time: ${formatDateWithTime(new Date().getTime())}`);
//             // tslint:disable-next-line:max-line-length
//             exec(`${getProperty('BITTERY_INFRASTRUCTURE_PATH')}/start
//             -lnds.sh ${lndUserDomainNamesString}`, (error: any, stdout: any, stderr: any) => {
//                 if (error) {
